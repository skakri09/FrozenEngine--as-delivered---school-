...
CSmpeg::CSmpeg() : log("SmpegPlayer", INFO)
{
	...
	GCPtr<CXmlResourcesImport> xmlImport = new CXmlResourcesImport(VIDEO_FILES, "XMLparser(SmpgPlayer)");
	int videoAmount = xmlImport->GetFilePathVector()->size();
	string filePath;
	for(int i = 0; i < videoAmount; ++i)
	{
		filePath = xmlImport->GetFilePathVector()->at(i);
		log << INFO << "Adding: " << filePath << " to videoMap." << endl;
		VideoFile* videoFile = new VideoFile;
		videoFile->filePath = filePath;
		videoFile->videoInfo = xmlImport->GetVideoInfoVector()->at(i);
		videoMap[xmlImport->GetFileNameVector()->at(i)] = videoFile;
	}
	log << INFO << "constructor successfull" << endl;
}
...
CSmpeg::~CSmpeg()
{
	...
	VideoMap::iterator mapIter;
	for(mapIter = videoMap.begin(); mapIter != videoMap.end(); mapIter++)
	{
		delete (*mapIter).second->videoInfo;
		(*mapIter).second->videoInfo = 0;
	}
...
string CSmpeg::getVideoPath(string videoName)
{
	if(videoMap.find(videoName) != videoMap.end())
	{
		log << INFO << "Successfully found: " << videoName << endl;
		return videoMap.find(videoName)->second->filePath;
	}
	else
	{
		log << ERRORX << "Could not find: " << videoName << endl;
		return "";
	}
}

VideoInfo* CSmpeg::GetVideoInfo()
{
	if(videoMap.find(currentlyLoadedVideo) != videoMap.end())
	{
		log << INFO << "Successfully found videoInfo for : " << currentlyLoadedVideo << endl;
		return videoMap.find(currentlyLoadedVideo)->second->videoInfo;
	}
	else
	{
		log << ERRORX << "Could not find videoInfo for: " << currentlyLoadedVideo << endl;
		return NULL;
	}
}
