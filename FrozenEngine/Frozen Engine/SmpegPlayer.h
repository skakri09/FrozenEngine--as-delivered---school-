/********************************************************************
	created:	2012/04/15
	created:	15:4:2012   22:30
	filename: 	C:\backup_test\Frozen Engine\SmpegPlayer.h
	file path:	C:\backup_test\Frozen Engine
	file base:	SmpegPlayer
	file ext:	h
...
typedef std::map<string, VideoFile*> VideoMap;
...
private:
	Logger log;
	//map with video filenames and locations
	VideoMap videoMap;
	
	// Returns the path of the string in filename, if it exists in map
	...
	// Holds the position of the videofile we're using in the videoMap
	string currentlyLoadedVideo;
